...
<link rel="alternate" type="application/rss+xml" title="Municipal Intelligence · Rankings de municipios" href="rss.xml">
...
@media (max-width:700px){
  ...
  #top h1{font-size:13px;margin-bottom:4px}
  #top h1 small{font-size:10px}
  #buscar{padding:7px 10px;font-size:13px}
...
<div id="side">
  ...
  <div id="sShare" style="display:none;margin:6px 0">
...
// instalacion PWA discreta (solo cuando el navegador lo permite y el usuario ha interactuado)
var installEvt=null;
window.addEventListener('beforeinstallprompt',function(e){
  e.preventDefault(); installEvt=e;
  document.getElementById('instalarBtn').style.display='inline';
});
document.getElementById('instalarBtn').addEventListener('click',function(e){
  e.preventDefault();
  if(installEvt){ installEvt.prompt(); installEvt=null; }
  else{ document.getElementById('instalarBtn').textContent='Añádela a la pantalla de inicio desde el menú del navegador'; }
});
...
function mostrar(m){
  ...
  if(m.sl){ sh.style.display='block'; shareBtn.textContent='🔗 Compartir este municipio'; }
  else { sh.style.display='none'; }
  try{history.replaceState(null,'','?m='+m.c);}catch(e){}
...
shareBtn.addEventListener('click',function(){if(actual)compartir(actual);});
// DEEP-LINK: ?m=CODIGO abre ese municipio; actualiza la URL al seleccionar
function muniDeURL(){
  var q=new URLSearchParams(location.search).get('m');
  if(!q)return;
  var m=catalogo.find(function(x){return x.c===q;});
  if(m)setTimeout(function(){irA(m);},600);
}
window.addEventListener('load',muniDeURL);
...
busq.addEventListener('input',function(){
  ...
  var r=catalogo.filter(m=>norm(m.n).includes(norm(q))||m.c.includes(q)).slice(0,40);
...
busq.addEventListener('keydown',function(e){
  if(e.key==='Enter'){var qn=norm(busq.value);var r=catalogo.filter(m=>norm(m.n)===qn);if(r.length)irA(r[0]);}
});
function irA(m){
...
cmpBus.addEventListener('input',function(){
  ...
  var r=catalogo.filter(m=>norm(m.n).includes(norm(q))).slice(0,25);
